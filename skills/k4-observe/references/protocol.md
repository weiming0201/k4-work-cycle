...
the new subject, boundary, cutoff, delta summary, observation completeness, and
initial additions. Iteration input states only the new cutoff, delta summary,
observation completeness, and any semantic updates, additions, or retirements.
Evidence is declared only on the changed, added, or retired meaning it supports;
the Tool derives the delta and Account evidence closures.
...
lens index, changed-evidence union, state indexes, and all envelope metadata.
Source references are the ordered unique union of evidence used by current
items and retirements; they are not a caller-maintained inventory. Non-external routes receive
...
observation surface preserves the Agent's bounded completeness estimate, claim
limit, and remaining angles, and generates the aggregate status,
Goal-candidate, gap, conflict, and unknown indexes plus the exact lens-to-item
index without selecting a Goal. Every declared lens has
